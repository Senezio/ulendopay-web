      ...
      <div v-if="pinModal.open" class="modal-overlay" @click.self="closePinModal">
        <div class="modal">
          ...
          <div v-if="pinModal.step === 'phone'" class="modal__body">
            <p class="modal__info">
              We'll send a 6-digit reset code to your registered phone number.
            </p>
            <div class="modal__field">
              <label class="modal__label">Phone Number</label>
              <input
                v-model="pinModal.phone"
                class="modal__input"
                type="tel"
                placeholder="+265 XXX XXX XXX"
                @keyup.enter="requestPinOtp"
              />
            </div>
          ...
          <div v-if="pinModal.step === 'otp'" class="modal__body">
            <p class="modal__info">
              Enter the 6-digit code sent to <strong>{{ pinModal.phone }}</strong>.
            </p>
            <input
              v-model="pinModal.otp"
              class="modal__input modal__input--otp"
              type="tel"
              placeholder="000000"
              maxlength="6"
              inputmode="numeric"
            ...
            <button
              class="modal__btn modal__btn--primary"
              :disabled="pinModal.loading || pinModal.otp.length < 6"
              @click="goToNewPin"
            >
              <span>Continue</span>
            </button>
            <button class="modal__btn modal__btn--ghost" @click="pinModal.step = 'phone'">
              ← Back
            </button>
          </div>
          ...
          <div v-if="pinModal.step === 'new'" class="modal__body">
            <div class="modal__field">
              ...
              <input
                v-model="pinModal.newPin"
                class="modal__input modal__input--pin"
                type="password"
                inputmode="numeric"
                maxlength="4"
                placeholder="••••"
          ...
          <div v-if="pinModal.step === 'done'" class="modal__body modal__body--center">
            ...
            <p class="modal__info">
              ...
              Please sign in with your new PIN.
            </p>
            <button class="modal__btn modal__btn--primary" @click="closePinModal">
              Back to Sign In
            </button>
          </div>
        ...
        <div class="modal">
          ...
          <div v-if="pwModal.step === 'email'" class="modal__body">
            ...
            <div class="modal__field">
              ...
              <input
                v-model="pwModal.email"
                class="modal__input"
                type="email"
                placeholder="name@email.com"
                @keyup.enter="requestPwOtp"
              />
            </div>
          ...
          <div v-if="pwModal.step === 'otp'" class="modal__body">
            <p class="modal__info">
              Enter the 6-digit code sent to <strong>{{ pwModal.email }}</strong>.
            </p>
            <input
              v-model="pwModal.otp"
              class="modal__input modal__input--otp"
            ...
            <button
              class="modal__btn modal__btn--primary"
              :disabled="pwModal.loading || pwModal.otp.length < 6"
              @click="goToNewPassword"
            >
              <span>Continue</span>
            </button>
            <button class="modal__btn modal__btn--ghost" @click="pwModal.step = 'email'">
              ← Back
            </button>
          </div>
          ...
          <div v-if="pwModal.step === 'done'" class="modal__body modal__body--center">
            ...
            <button class="modal__btn modal__btn--primary" @click="closePwModal">
              Back to Sign In
            </button>
          </div>
...
function openForgotPin() {
  // Pre-fill phone if user already typed it in the login form
  pinModal.value = {
...
async function submitPinReset() {
  ...
  try {
    ...
    pinModal.value.error = errors
      ? Object.values(errors).flat()[0]
      : (e.response?.data?.message || 'Reset failed. The code may have expired.')
    // If OTP was wrong, send back to re-enter it
...
.modal__title { font-size: 16px; font-weight: 700; letter-spacing: -0.02em; }
...
.modal__label { font-size: 12px; font-weight: 600; color: var(--text-secondary); }
...
.modal__success-icon {
  font-size: 52px; color: var(--success); line-height: 1;
}
</style>
